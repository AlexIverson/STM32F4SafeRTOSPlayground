...
portspecCOUNTSEM_TASK_ZERO_DATA_SECTION static volatile xCountSemStruct axParameters[ countNUM_TEST_TASKS ] =
{
    { NULL, 0U, 0U },
    { NULL, 0U, 0U },
    { NULL, 0U, 0U }
...
void vCountingSemaphoreTimerHandler( void )
{
    portUnsignedBaseType uxSavedInterruptStatus;

    portBaseType xStatus;

    xSemaphoreHandle xSemaTimerHandle = axParameters[ 2 ].xSemaphore;
...
portBaseType xStartCountingSemaphoreTasks( void )
{
    ...
    xTaskParameters xTestTask1Parameters =
    {
        ...
        ( void * ) &( axParameters[ 0 ] ),      /* Parameters */
    ...
    xTaskParameters xTestTask2Parameters =
    {
        ...
        portspecCOUNTSEM_TASK_STACK_SIZE,       /* Stack depth bytes */
        ( void * ) &( axParameters[ 1 ] ),      /* Parameters */
        taskIDLE_PRIORITY,                      /* Priority */
        NULL,                                   /* TLS Object */
    ...
    xTaskParameters xTestTask3Parameters =
    {
        ...
        ( void * ) &( axParameters[ 2 ] ),      /* Parameters */
        taskIDLE_PRIORITY,                      /* Priority */
        NULL,                                   /* TLS Object */
    ...
    if( pdPASS == xStatus )
    {
        xStatus = xSemaphoreCreateCounting( countMAX_COUNT_VALUE,
                                            countMAX_COUNT_VALUE,
                                            acCountSem1,
                                            ( xSemaphoreHandle * ) &( axParameters[ 0 ].xSemaphore ) );
    }

    if( ( xStatus == pdPASS ) && ( axParameters[ 0 ].xSemaphore != NULL ) )
    {
#if ( configQUEUE_REGISTRY_SIZE > 0 )
        /* Add to Queue Registry */
        vQueueAddToRegistry( axParameters[ 0 ].xSemaphore, "CountingSemaphore #1" );
#endif

        axParameters[ 0 ].uxExpectedStartCount = countSTART_AT_MAX_COUNT;
        axParameters[ 0 ].uxLoopCounter = 0;
    ...
    if( pdPASS == xStatus )
    {
        xStatus = xSemaphoreCreateCounting( countMAX_COUNT_VALUE,
                                            0,
                                            acCountSem2,
                                            ( xSemaphoreHandle * ) &( axParameters[ 1 ].xSemaphore ) );
    }

    if( ( xStatus == pdPASS ) && ( axParameters[ 1 ].xSemaphore != NULL ) )
    {
#if ( configQUEUE_REGISTRY_SIZE > 0 )
        /* Add to Queue Registry. */
        vQueueAddToRegistry( axParameters[ 1 ].xSemaphore, "CountingSemaphore #2" );
#endif

        axParameters[ 1 ].uxExpectedStartCount = 0;
        axParameters[ 1 ].uxLoopCounter = 0;
    ...
    if( pdPASS == xStatus )
    {
        xStatus = xSemaphoreCreateCounting( 1, 1, acCountSem3,
                                            ( xSemaphoreHandle * ) &( axParameters[ 2 ].xSemaphore ) );
    }

    if( ( xStatus == pdPASS ) && ( axParameters[ 2 ].xSemaphore != NULL ) )
    {
#if ( configQUEUE_REGISTRY_SIZE > 0 )
        /* Add to Queue Registry. */
        vQueueAddToRegistry( axParameters[ 2 ].xSemaphore, "CountingSemaphore #3" );
#endif

        axParameters[ 2 ].uxExpectedStartCount = 0;
        axParameters[ 2 ].uxLoopCounter = 0;
...
static void prvInterruptSemaphoreTask( void *pvParameters )
{
    ...
    for( ;; )
    {
        ...
        xStatus = xSemaphoreTake( pxParameter->xSemaphore, safertosapiMAX_DELAY );
...
portBaseType xAreCountingSemaphoreTasksStillRunning( void )
{
    ...
    if( uxLastCount0 == axParameters[ 0 ].uxLoopCounter )
    {
        xReturn = pdFAIL;
    }
    else
    {
        uxLastCount0 = axParameters[ 0 ].uxLoopCounter;
    }

    if( uxLastCount1 == axParameters[ 1 ].uxLoopCounter )
    {
        xReturn = pdFAIL;
    }
    else
    {
        uxLastCount1 = axParameters[ 1 ].uxLoopCounter;
    }

    if( uxLastCount2 == axParameters[ 2 ].uxLoopCounter )
    {
        xReturn = pdFAIL;
    }
    else
    {
        uxLastCount2 = axParameters[ 2 ].uxLoopCounter;
    }
