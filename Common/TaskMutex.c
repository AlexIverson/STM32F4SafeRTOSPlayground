...
#define taskmutexCONTROLLING_TASK_PRIORITY  ( taskIDLE_PRIORITY + 2U )
#define taskmutexBLOCKING_TASK_PRIORITY     ( taskIDLE_PRIORITY + 1U )
#define taskmutexPOLLING_TASK_PRIORITY      ( taskIDLE_PRIORITY + 0U )

/* The recursive call depth. */
#define taskmutexMAX_COUNT                  ( 10U )

/* Misc. */
#define taskmutexSHORT_DELAY                ( ( portTickType ) 20U )
#define taskmutexNO_DELAY                   ( ( portTickType ) 0U )
#define taskmutex50ms_DELAY                 ( ( portTickType ) 50U )
...
static void prvRecursiveMutexControllingTask( void *pvParameters )
{
    ...
    for( ;; )
    {
        ...
        for( uxIndex = 0U; uxIndex < taskmutexMAX_COUNT; uxIndex++ )
        {
...
static void prvRecursiveMutexBlockingTask( void *pvParameters )
{
    ...
    for( ;; )
    {
         ...
         * task is suspended. */
        if( pdPASS == xMutexTake( xMutex, safertosapiMAX_DELAY ) )
        {
        ...
        uxBlockingCyclesTmp = uxBlockingCycles + 1U;
