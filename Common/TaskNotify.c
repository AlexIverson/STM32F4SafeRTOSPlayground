...
#define notifyBIT_0             ( ( portUnsignedBaseType ) 0x01U )
#define notifyBIT_1             ( ( portUnsignedBaseType ) 0x02U )

#define notifyTIMER_ID          ( 71 )

#define notifyMIN_PERIOD        ( ( portTickType ) 10U )
...
static void prvSingleTaskTests( void )
{
     ...
     */
    xReturnValue = xTaskNotifySend( xTaskToNotify, taskNOTIFICATION_SET_VAL_WITHOUT_OVERWRITE, uxFirstNotifiedValueConst );
    ...
    xReturnValue = xTaskNotifySend( xTaskToNotify, taskNOTIFICATION_SET_VAL_WITHOUT_OVERWRITE, uxSecondNotifiedValueConst );
     ...
     */
    xReturnValue = xTaskNotifySend( xTaskToNotify, taskNOTIFICATION_SET_VAL_WITH_OVERWRITE, uxFirstNotifiedValueConst );
    if( pdPASS != xReturnValue )
    {
        xNotifyErrorStatus = pdFAIL;
    }

    xReturnValue = xTaskNotifySend( xTaskToNotify, taskNOTIFICATION_SET_VAL_WITH_OVERWRITE, uxSecondNotifiedValueConst );
    if( pdPASS != xReturnValue )
    {
        xNotifyErrorStatus = pdFAIL;
    }
     ...
     * back should remain at uxSecondNotifiedValueConst.
     */
     ...
     * is as expected - which should be uxSecondNotifiedValueConst plus how
...
static void prvNotifiedTask( void *pvParameters )
{
    ...
    portTickType xMaxPeriod = ( xAverageNotifyPeriod << 1U ) - xMinPeriod;
    ...
    for( ;; )
    {
        ...
        xReturnValue = xTimerChangePeriod( xNotifyingTimerHandle, xPeriod, safertosapiMAX_DELAY );
        if( pdPASS != xReturnValue )
        {
            xNotifyErrorStatus = pdFAIL;
        }

        xReturnValue = xTimerStart( xNotifyingTimerHandle, safertosapiMAX_DELAY );
        if( pdPASS != xReturnValue )
        {
            xNotifyErrorStatus = pdFAIL;
        }
        ...
        if( xTaskNotifyWait( 0x00, notifyMAX_VALUE, &uxNotificationValue, safertosapiMAX_DELAY ) == pdPASS )
        {
            uxNotificationsReceived += uxNotificationValue;
        }
...
void vNotifyTaskFromISR( void )
{
    portspecCOMMON_PRIV_DATA_SECTION static portUnsignedBaseType uxCallCount = 0U;
    ...
    if( NULL != xNotifyingTimerHandle )
    {
        ...
        if( uxCallCount >= xAverageNotifyPeriod )
        {
            ...
            ( void ) xTaskNotifySendFromISR( xTaskToNotify, taskNOTIFICATION_INCREMENT, 0 );
            uxNotificationsSent++;
...
{
    portspecCOMMON_PRIV_DATA_SECTION static portUnsignedBaseType uxLastNotifyCycleCount = 0U;
...
static portUInt32Type prvRand( void )
{
    const portUInt32Type ulMultiplier = 0x015A4E35UL;
    const portUInt32Type ulIncrement = 1UL;

    /* Utility function to generate a pseudo random number. */
    ulNextRand = ( ulMultiplier * ulNextRand ) + ulIncrement;
    return ( ( ulNextRand >> 16U ) & 0x7FFFUL );
}
/*---------------------------------------------------------------------------*/
